...
/*   Updated: 2024/02/27 09:13:48 by yoropeza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float ft_hinter(t_data *data, float angl)
{
	...
	if ((ft_circle(angl, 'y') && x_step > 0) || (!ft_circle(angl, 'y') && x_step < 0))
		x_step *= -1;
	...
	return (sqrt(pow(h_x - data->ply->pos->x, 2) + pow(h_y - data->ply->pos->y, 2)));
}
...
float ft_vinter(t_data *data, float angl)
{
	...
	if ((ft_circle(angl, 'x') && y_step < 0) || (!ft_circle(angl, 'x') && y_step > 0))
		y_step *= -1;
	...
	return (sqrt(pow(v_x - data->ply->pos->x, 2) + pow(v_y - data->ply->pos->y, 2)));
}

void	ft_raycasting(t_data *data, int ray)
{
	t_point start;
	t_point end;
