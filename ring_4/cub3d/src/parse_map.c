...
/*   Updated: 2024/03/04 21:33:07 by angalsty         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void  ft_flood_fill(t_data *data, int y, int x)
{
	...
	if (x < 0 || y < 0 || x >= data->structure->size_x + 2 || y >= data->structure->size_y + 2)
		return ;
	if (data->structure->copy_map[y][x] != '1' && data->structure->copy_map[y][x] != '2'
	&& data->structure->copy_map[y][x] != '3' && data->structure->copy_map[y][x] != '\0')
	{
		...
		ft_flood_fill(data, y, x - 1);
		//printf("data->structure->copy_map[y][x] = %c\n", data->structure->copy_map[y][x]);
	}
	ft_check_flood(data);
	...
	while (data->structure->map[y] != NULL)
	{
		...
		while (data->structure->map[y][x] != '\0')
		{
			if(data->structure->map[y][x] != '\n')
				data->structure->copy_map[y + 1][x + 1] = data->structure->map[y][x];
			x++;
		}
		y++;
	}
...
void  ft_copy_map(t_data *data)
{
	...
	//data->structure->copy_map = (char **)malloc(sizeof(char *) * (data->structure->size_y + 3));
	data->structure->copy_map =ft_calloc((data->structure->size_y + 3) * sizeof(char),
			(data->structure->size_x + 3) * sizeof(char));
	line = malloc(sizeof(char) * (data->structure->size_x + 3));
	if (!data->structure->copy_map)
		exit_error("Malloc error\n");
	// while (data->structure->map[i] != NULL)
	// {
	// 	data->structure->copy_map[i] = ft_strdup(data->structure->map[i]);
	// 	i++;
	// }
	// data->structure->copy_map[i] = NULL;
